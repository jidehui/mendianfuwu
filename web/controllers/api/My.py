...
def myOrderList():
    ...
    if pay_order_list:
        ...
        shop_ids = selectFilterObj(pay_order_item_list, "shop_id")
...
def myOrderInfo():
    ...
    if pay_order_items:
        shop_ids = selectFilterObj(pay_order_items, "shop_id")
        shop_map = getDictFilterField(Shop, Shop.id, "id", shop_ids)
...
def myCommentAdd():
    ...
    pay_order_items = PayOrderItem.query.filter_by(pay_order_id=pay_order_info.id).all()
    shop_ids = selectFilterObj(pay_order_items, "shop_id")
    tmp_shop_ids_str = '_'.join(str(s) for s in shop_ids if s not in [None])
    model_comment = MemberComments()
    model_comment.shop_ids = "_%s_" % tmp_shop_ids_str
