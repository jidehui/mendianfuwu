...
def orderInfo():
    ...
    for item in params_goods_list:
        shop_dic[item['id']] = item['number']

    shop_ids = shop_dic.keys()
    shop_list = Shop.query.filter(Shop.id.in_(shop_ids)).all()
    data_shop_list = []
    ...
    if shop_list:
        for item in shop_list:
            tmp_data = {
                ...
                'number': shop_dic[item.id]
            }
            pay_price = pay_price + item.price * int(shop_dic[item.id])
    ...
    resp['data']['shop_list'] = data_shop_list
