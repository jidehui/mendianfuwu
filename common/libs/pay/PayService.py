...
from common.models.shop.ShopSaleChangeLog import ShopSaleChangeLog
...
class PayService():
    ...
    def createOrder(self, member_id, items=None, params=None):
        ...
        try:
            ...
            tmp_shop_list = db.session.query(Shop).filter(Shop.id.in_(shop_ids)) \
                .with_for_update().all()

            tmp_shop_stock_mapping = {}
            for tmp_item in tmp_shop_list:
                tmp_shop_stock_mapping[tmp_item.id] = tmp_item.stock
            ...
            # db.session.flush()
            for item in items:
                tmp_left_stock = tmp_shop_stock_mapping[item['id']]
                ...
                tmp_ret = Shop.query.filter_by(id=item['id']).update({
    ...
    def closeOrder(self, pay_order_id=0):
        ...
        if pay_order_items:
            ...
            for item in pay_order_items:
                tmp_shop_info = Shop.query.filter_by(id=item.shop_id).first()
                if tmp_shop_info:
                    tmp_shop_info.stock = tmp_shop_info.stock + item.quantity
                    tmp_shop_info.updated_time = getCurrentDate()
                    db.session.add(tmp_shop_info)
                    db.session.commit()
                    ShopService.setStockChangeLog(item.shop_id, item.quantity, "订单取消")
    ...
    def orderSuccess(self, pay_order_id=0, params=None):
        try:
            ...
            for order_item in pay_order_items:
                ...
                tmp_model_sale_log.shop_id = order_item.shop_id
